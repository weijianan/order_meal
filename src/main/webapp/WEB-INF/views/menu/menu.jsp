...
<div class="content">
        ...
        <div class="container-fluid">
            ...
            <div class="row-fluid">
                <div class="btn-toolbar">
                    <a href="<%=basePath %>menu?flag=menuAdd" role="button" class="btn btn-primary" data-toggle="modal">
                    ...
                    <c:forEach items="${menulist}" var="menu" begin="0"  end="3">
                        ...
                        <li class="span3">
                            <div class="thumbnail">
                                ...
                                <h3>${menu.name}</h3>                             
                                <a href="<%=basePath %>menu?flag=menuPre&id=${menu.id}" class="btn btn-primary"> 预 览</a>
                                <a  class="btn btn-info" href="<%=basePath %>menu?flag=menuEdit&id=${menu.id}">
      ...
      <ul>
    	<c:if test="${pre=='yes'}">
        <li><a href="<%=basePath %>menu?flag=menu&pageNow=${pNow-1}">前一页</a></li>
    	</c:if>
        <c:forEach var="i" begin="1" end="${pageCount}">
        <li>
        <a href="<%=basePath %>menu?flag=menu&pageNow=${i }">${i }</a>
        </li>
        </c:forEach>
        <c:if test="${next=='yes'}">
        <li><a href="<%=basePath %>menu?flag=menu&pageNow=${pNow+1}">后一页</a></li>
        </c:if>
    </ul>
    </div>
</div>  
			...
			<script type="text/javascript">
				function del(obj) {
					...
					var delurl = "<%=basePath %>menu?flag=delMenu&delid="
							+ delid + "&pageNow=" + pNow;
